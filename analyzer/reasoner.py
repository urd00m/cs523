...
def pwsc_is_exploitable(t : pd.Series):
    exploitable = True

    fail_reasons = []
    required_solutions = []

    for b in pwsc_basic_checks:
        if not b(t):
            exploitable = False
            fail_reasons.append(b.__name__)

    for c in pwsc_advanced_checks:
        if t[f'{c["problem"].__name__}{"" if not with_branches else "_w_branches"}']:
            if t[f'{c["solution"].__name__}{"" if not with_branches else "_w_branches"}']:
                required_solutions.append(c['solution'].__name__)
            else:
...
def run(in_csv, out_csv):
    ...
    # --------------------------------------------------------------------------
    # Perform the checks for the PWSC attack
    print("[-] Performing exploitability analysis assuming the PWSC covert channel...")
    with_branches = False
    ...
    df[['exploitable_w_pwsc', 'required_solutions_w_pwsc',
        'fail_reasons_w_pwsc']] = df.apply(pwsc_is_exploitable, axis=1, result_type="expand")

    print(f"Found {len(df[df['exploitable_w_pwsc'] == True])} exploitable gadgets!")
